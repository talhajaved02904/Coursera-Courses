...
<li> The middle 50\% of the houses range between approximately 1,126 square feet and 1,742.7 square feet. This is false </li>
...
samp1 %>%
  summarise(mu_r = mean(area), pop_med_r = median(area), 
            sigma_r = sd(area), pop_iqr_r = IQR(area),
            pop_min_r = min(area), pop_max_r = max(area),
            pop_q1_r = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3_r = quantile(area, 0.75))  # third quartile, 75th percentile
...
<li> Sample size of 1000. One with greater population</li>
</ol>
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. This is true. </li>
...
<ol>
<li> decreases. This is correct </li>
<li> increases </li>
...
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). This is false. </li>
