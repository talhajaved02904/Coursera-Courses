...
* The probability that the true proportion of females lies in this interval is 0.95. This is correct
...
* Gamma($a = 22$, $b = 6$) . This is correct
* Gamma($a = 18$, $b = 5$)
* Gamma($a = 18$, $b = 6$)
* Gamma($a = 19$, $b = 8$)
</div>
...
* Gamma($\alpha = 8119$, $\beta = 5001$). This is correct
...
alpha <- 5
beta <- 1
alpha <- alpha + sum(brfss$fruit_per_day)
beta <- beta + length(brfss$fruit_per_day)
alpha
beta
```
...
which recommend consuming 5 servings of vegetables per day. No
</div>
...
alpha <- 5
beta <- 1
alpha <- alpha + sum(brfss$vege_per_day)
beta <- beta + length(brfss$vege_per_day)
alpha
beta
