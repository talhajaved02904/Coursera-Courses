...
* Observational study <- This is correct
* Experiment
</div>
...
* 10 of the respondents make strictly less than 300 dollars per week <- This one
...
* Each additional year of education increases weekly wages by $60.21. <- This one
...
* The residuals appear to be randomly distributed around 0. <- This one
...
your answer to the previous exercise change? No
</div>
...
summary(m_wage_educ)
ggplot(m_wage_educ, aes(x=fitted(m_wage_educ), y=residuals(m_wage_educ)))+geom_point()+geom_abline()
...
residuals reasonable? Seems reasonable
</div>

```{r E-log-resid}
# Type your code for Exercise 2 here.
plot(m_lwage_iq, which = 1)
...
* Case 616 has a probably of close to 1 that it an outlier under the normal error model for regressing `lwage` on `iq` <- This one
...
* The married black man <- This one
